...
func (repo *OrderItemRepository) CreateCommand(ctx context.Context, orderItem *entity.OrderItem, tx *sqlx.Tx) error {
	insertQuery := `INSERT INTO order_items(order_id, product_id, number_of_boxes, spec, quantity, selling_price, discount, final_amount) VALUES (:order_id, :product_id, :number_of_boxes, :spec, :quantity, :selling_price, :discount, :final_amount)`
